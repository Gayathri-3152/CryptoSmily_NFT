...
class App extends Component {
  ...
  checkIfCanMint = (lastMintTime) => {
    ...
    const interval = setInterval(() => {
      ...
      if (diff < 0) {
        ...
        mintBtn.innerHTML = "Mint My Crypto Smily";
  ...
  setBidBtnTimer = () => {
    const mintBtn = document.getElementById("timerBtn");
    if (mintBtn !== undefined && mintBtn !== null) {
      this.setState({
        lastMintTime: localStorage.getItem(this.state.cryptoBoys.every),
      });
      this.state.lastMintTime === undefined || this.state.lastMintTime === null
        ? (mintBtn.innerHTML = "Change Price")
        : this.checkIfCanBid(parseInt(this.state.lastMintTime));
    }
  };

  checkIfCanBid = (lastMintTime) => {
    const mintBtn = document.getElementById("timerBtn");
    ...
    const interval = setInterval(() => {
      ...
      if (diff < 0) {
        ...
        mintBtn.setAttribute("disabled", true);
        mintBtn.innerHTML = "Bidding completed";       
        localStorage.removeItem(this.state.cryptoBoys.every);
        clearInterval(interval);
      } else {
        buyBtn.setAttribute("disabled", true);
        const minutes = Math.floor((diff % (1000 * 60 * 60)) / (1000 * 60));
        const seconds = Math.floor((diff % (1000 * 60)) / 1000);
        mintBtn.innerHTML = `You can bid in ${minutes}m ${seconds}s`;
      }
    }, 1000);
  };
  loadWeb3 = async () => {
  ...
  mintMyNFT = async (colors, name, tokenPrice) => {
    ...
    if (colorsUsed.length === 0 && !nameIsUsed) {
      ...
      if (nameIsUsed) {
        ...
        this.setState({ colorIsUsed: true });//1
  ...
  changeBidTokenPrice = (tokenId, newPrice) => {
    this.setState({ loading: true });
    const newTokenPrice = window.web3.utils.toWei(newPrice, "Ether");
    this.state.cryptoBoysContract.methods
      .changeBidTokenPrice(tokenId, newTokenPrice)
      .send({ from: this.state.accountAddress })
      .on("confirmation", () => {
        localStorage.setItem(this.state.cryptoBoys.every, new Date().getTime());
          this.setState({ loading: false });
          window.location.reload();
      });
  };
              ...
              <Route
                ...
                render={() => (
                  <AllCryptoBoys
                    ...
                    changeBidTokenPrice={this.changeBidTokenPrice}
                    toggleforBidding={this.toggleforBidding}
                    buyCryptoBoy={this.buyCryptoBoy}
                    setBidBtnTimer={this.setBidBtnTimer}
