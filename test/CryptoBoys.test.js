...
contract("Crypto Boys", async (accounts) => {
  ...
  describe("Deployment", async () => {
    ...
    it("has a name", async () => {
      ...
      assert.equal(name, "Crypto Smily Collection");
    });
 ...
 /*
   ...
   */
    ...
    /*it("allows users to toggle between setting the token for sale or not for sale", async () => {
      ...
      result = await cryptoBoys.toggleForSale(1, { from: accounts[0] });
      ...
      await cryptoBoys.toggleForSale(94, { from: accounts[0] }).should.be
        .rejected;
    ...
    it("allows users to toggle between setting the token for bidding or not for bidding", async () => {
      let cryptoboy;
      cryptoboy = await cryptoBoys.allCryptoBoys(1, {
        from: accounts[0],
      });
      assert.equal(cryptoboy.forBidding, false);

      result = await cryptoBoys.toggleforBidding(1, { from: accounts[0] });

      cryptoboy = await cryptoBoys.allCryptoBoys(1, {
        from: accounts[0],
      });
      assert.equal(cryptoboy.forBidding, true);

    
      await cryptoBoys.toggleforBidding(1, {
        from: 0x0000000000000000000000000000000000000000,
      }).should.be.rejected;

      await cryptoBoys.toggleforBidding(94, { from: accounts[0] }).should.be
        .rejected;

      await cryptoBoys.toggleforBidding(1, { from: accounts[8] }).should.be
        .rejected;
    });

    it("allows bidders to change bidding price", async () => {
      let cryptoBoyPrice;
      cryptoBoyPrice = await cryptoBoys.allCryptoBoys(1, {
        from: accounts[0],
      });
      assert.equal(web3.utils.fromWei(cryptoBoyPrice.price, "ether"), 2);
      assert.equal(web3.utils.fromWei(cryptoBoyPrice.biddingPrice, "ether"), 0);


      result = await cryptoBoys.changeBidTokenPrice(
        1,
      ...
      assert.equal(web3.utils.fromWei(cryptoBoyPrice.biddingPrice, "ether"), 3);
      assert.equal(cryptoBoyPrice.currentBidder, accounts[2]);
      ...
      assert.equal(web3.utils.fromWei(cryptoBoyPrice.biddingPrice, "ether"), 4);
      assert.equal(cryptoBoyPrice.currentBidder, accounts[3]);

     
      await cryptoBoys.changeBidTokenPrice(1, web3.utils.toWei("6", "Ether"), {
        from: 0x0000000000000000000000000000000000000000,
      }).should.be.rejected;

      await cryptoBoys.changeBidTokenPrice(82, web3.utils.toWei("6", "Ether"), {
        from: accounts[4],
      }).should.be.rejected;
      ...
      await cryptoBoys.changeBidTokenPrice(1, web3.utils.toWei("3", "Ether"), {
        from: accounts[3],
      }).should.be.rejected;
    ...
    it("allows users to buy token for specified ethers", async () => {
      ...
      assert.equal(cryptoBoy.currentOwner, accounts[0]);
      assert.equal(cryptoBoy.currentBidder, accounts[3]);

      await cryptoBoys.buyToken(1, {
        from: 0x0000000000000000000000000000000000000000,
      ...
      const newTokenOwner = await cryptoBoys.getTokenOwner(1);
      assert.equal(newTokenOwner, accounts[3]);
      ...
      assert.equal(cryptoBoy.currentOwner, accounts[3]);
